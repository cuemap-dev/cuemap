...
async fn test_single_url_chunking() {
    ...
    match result {
        Ok(chunks) => {
            println!(
                "Single URL test: {} chunks extracted from {}",
                chunks.len(),
                test_url
            );
            assert!(
                !chunks.is_empty(),
                "Should extract at least one chunk from example.com"
            );
            ...
            for chunk in &chunks {
                ...
                assert!(
                    ...
                    "Chunk should have structural cues"
                );
            }

            // Verify domain cue is present
            let has_domain_cue = chunks
                .iter()
                .any(|c| c.structural_cues.iter().any(|s| s.contains("domain:")));
...
async fn test_recursive_crawl_depth_1() {
    ...
    let engine = Arc::new(MultiTenantEngine::new());
    ...
    match result {
        Ok(crawl_result) => {
            ...
            assert!(
                !crawl_result.memory_ids.is_empty(),
                "Should create memory chunks"
            );
            ...
            if let Some(session) = job_queue.get_session("test-project") {
                ...
                println!(
                    "  Writes: {}/{}",
                    progress.writes_completed, progress.writes_total
                );
...
async fn test_job_phase_ordering() {
    ...
    if let Ok(memory_ids) = result {
        if let Some(session) = job_queue.get_session("phase-test") {
            ...
            println!(
                "Phase test passed: {} writes completed",
