...
fn test_chunking_coverage_all_file_types() {
    ...
    let mut results: std::collections::HashMap<String, (usize, usize)> =
        std::collections::HashMap::new();

    let files = collect_files(base_dir);
    assert!(
        !files.is_empty(),
        "Should find test files in data/agent-test/"
    );
        ...
        if let Ok(content) = fs::read_to_string(&file_path) {
            ...
            for chunk in &chunks {
                assert!(
                    ...
                    "Chunk content should not be empty for {:?}",
                    file_path
                );
                assert!(
                    !chunk.structural_cues.is_empty(),
                    "Chunk should have structural cues for {:?}",
                    file_path
                );
            }
        }
    ...
    for (category, (file_count, chunk_count)) in &results {
        println!(
            "{}: {} files -> {} chunks",
            category, file_count, chunk_count
        );
    }

    // Assert minimum coverage
    assert!(
        results.get("whatsapp").map(|r| r.0).unwrap_or(0) >= 1,
        "Should process WhatsApp files"
    );
    assert!(
        results.get("instagram").map(|r| r.0).unwrap_or(0) >= 1,
        "Should process Instagram files"
    );
    assert!(
        results.get("programming").map(|r| r.0).unwrap_or(0) >= 5,
        "Should process programming files"
    );
    assert!(
        results.get("markdown").map(|r| r.0).unwrap_or(0) >= 1,
...
fn test_whatsapp_content_extraction() {
    ...
    if let Ok(content) = fs::read_to_string(&wa_path) {
        ...
        // Verify platform cue
        let has_platform = chunks
            .iter()
            .any(|c| c.structural_cues.contains(&"platform:whatsapp".to_string()));
        assert!(has_platform, "Should have platform:whatsapp cue");

        // Verify participant/sender extraction
        let has_senders = chunks
            .iter()
            .any(|c| c.structural_cues.iter().any(|s| s.starts_with("sender:")));
...
fn test_programming_file_extraction() {
    ...
    for (path_str, expected_lang_cue) in test_cases {
        ...
        if let Ok(content) = fs::read_to_string(&path) {
            ...
            let has_lang = chunks
                .iter()
                .any(|c| c.structural_cues.contains(&expected_lang_cue.to_string()));
            assert!(
                has_lang,
                "Should have {} cue for {}",
                expected_lang_cue, path_str
            );
        }
    }
}
...
fn test_openapi_detection() {
    ...
    if let Ok(content) = fs::read_to_string(&api_path) {
        ...
        let has_yaml_cue = chunks.iter().any(|c| {
            c.structural_cues
                .iter()
                .any(|s| s.contains("yaml") || s.contains("openapi"))
        });
...
fn test_csv_chunking() {
    ...
    if let Ok(content) = fs::read_to_string(&csv_path) {
        ...
        assert!(
            has_row_content,
            "CSV chunks should contain comma-separated data"
        );
    }
}
...
fn test_markdown_chunking() {
    ...
    if let Ok(content) = fs::read_to_string(&md_path) {
        ...
        let has_heading = chunks.iter().any(|c| {
            c.structural_cues
                .iter()
                .any(|s| s.contains("heading") || s.contains("markdown"))
        });
...
fn test_instagram_chrome_youtube_extraction() {
    ...
    if let Ok(content) = fs::read_to_string(&ig_path) {
        ...
        let has_ig = chunks.iter().any(|c| {
            c.structural_cues
                .contains(&"platform:instagram".to_string())
        });
    ...
    if let Ok(content) = fs::read_to_string(&yt_path) {
        ...
        assert!(
            chunks.len() > 0 || content.len() < 100,
            "YouTube should produce chunks for non-trivial content"
        );
    }
}
