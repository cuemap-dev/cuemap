...
fn test_memory_payload_integration() {
    ...
    // Test Encrypted Flow
    let payload =
        Memory::<MainStats>::create_payload(content, Some(&key)).expect("create payload failed");
    let memory = Memory::<MainStats>::new(payload, None);

    let accessed = memory
        .access_content(Some(&key))
        .expect("access content failed");
    ...
    let payload_plain =
        Memory::<MainStats>::create_payload(content, None).expect("create plain payload failed");
    let memory_plain = Memory::<MainStats>::new(payload_plain, None);

    let accessed_plain = memory_plain
        .access_content(None)
        .expect("access plain failed");
...
fn test_engine_integration() {
    ...
    let engine = CueMapEngine::<MainStats>::with_key(Some(key));
